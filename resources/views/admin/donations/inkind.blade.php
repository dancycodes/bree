...
@section('title', 'Don en nature')
@section('page_title', 'Don en nature')
@section('breadcrumb', 'Dons › Nature › ' . $inkind->donor_name)
...
@section('content')
    ...
    @php
        $statusLabels = [
            ...
            'accepted' => ['label' => 'Accepté', 'bg' => 'rgba(22,163,74,0.1)', 'color' => '#15803d'],
            'declined' => ['label' => 'Refusé', 'bg' => 'rgba(220,38,38,0.1)', 'color' => '#dc2626'],
        ];
        $statusMeta = $statusLabels[$inkind->status] ?? ['label' => $inkind->status, 'bg' => '#f1f5f9', 'color' => '#64748b'];
    @endphp

    <div x-data="{ status: '{{ $inkind->status }}', adminNotes: {{ Js::from($inkind->admin_notes ?? '') }} }">
            ...
            <div class="lg:col-span-2 space-y-4">
                ...
                <div class="bg-white rounded-2xl shadow-sm p-6" style="border: 1px solid #e2e8f0;">
                    ...
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide mb-1" style="color: #94a3b8;">Nom</dt>
                            <dd class="text-sm font-medium" style="color: #1e293b;">{{ $inkind->donor_name ?: '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide mb-1" style="color: #94a3b8;">Organisation</dt>
                            <dd class="text-sm" style="color: #374151;">{{ $inkind->organization ?: '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide mb-1" style="color: #94a3b8;">Email</dt>
                            <dd class="text-sm" style="color: #374151;">{{ $inkind->email ?: '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide mb-1" style="color: #94a3b8;">Téléphone</dt>
                            <dd class="text-sm" style="color: #374151;">{{ $inkind->phone ?: '—' }}</dd>
                        </div>
                    </dl>
                </div>
                ...
                {{-- Donation details --}}
                <div class="bg-white rounded-2xl shadow-sm p-6" style="border: 1px solid #e2e8f0;">
                    <h3 class="text-sm font-semibold mb-4" style="color: #1e293b;">Détails du don en nature</h3>
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide mb-1" style="color: #94a3b8;">Type de don</dt>
                            <dd class="text-sm font-medium" style="color: #374151;">{{ $inkind->donation_type ?: '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide mb-1" style="color: #94a3b8;">Valeur estimée</dt>
                            <dd class="text-lg font-bold" style="color: #c80078;">
                                @if ($inkind->estimated_value)
                                    {{ number_format((float) $inkind->estimated_value, 0, ',', ' ') }} XAF
                                @else
                                    —
                                @endif
                            </dd>
                        ...
                        <div>
                            ...
                            <dd class="text-sm" style="color: #374151;">{{ $inkind->programme ?: '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide mb-1" style="color: #94a3b8;">Disponibilité</dt>
                            <dd class="text-sm" style="color: #374151;">{{ $inkind->availability ?: '—' }}</dd>
                        </div>
                        <div>
                            <dt class="text-xs font-medium uppercase tracking-wide mb-1" style="color: #94a3b8;">Date de soumission</dt>
                            <dd class="text-sm" style="color: #374151;">{{ $inkind->created_at->format('d/m/Y à H:i') }}</dd>
                        </div>
                        @if ($inkind->description)
                            <div class="sm:col-span-2">
                                <dt class="text-xs font-medium uppercase tracking-wide mb-1" style="color: #94a3b8;">Description</dt>
                                <dd class="text-sm" style="color: #374151;">{{ $inkind->description }}</dd>
                            </div>
                        @endif
                    </dl>
                </div>
            ...
            <div class="space-y-4">
                ...
                @can('donations.edit')
                    <div class="bg-white rounded-2xl shadow-sm p-5 space-y-2" style="border: 1px solid #e2e8f0;">
                        ...
                        <button x-show="status === 'pending_review'"
                                @click="status='accepted'; $action.patch('{{ route('admin.donations.inkind.status', $inkind) }}', { include: ['status','adminNotes'] })"
                            ...
                            <span x-show="!$fetching()">Accepter</span>
                            <span x-show="$fetching()">En cours…</span>
                        </button>

                        <button x-show="status === 'pending_review'"
                                @click="status='declined'; $action.patch('{{ route('admin.donations.inkind.status', $inkind) }}', { include: ['status','adminNotes'] })"
                                class="w-full inline-flex items-center justify-center px-4 py-2.5 rounded-xl text-sm font-semibold text-white"
                                style="background-color: #dc2626;">
                            <span x-show="!$fetching()">Refuser</span>
                            <span x-show="$fetching()">En cours…</span>
                        </button>

                        <button x-show="status !== 'pending_review'"
                                @click="status='pending_review'; $action.patch('{{ route('admin.donations.inkind.status', $inkind) }}', { include: ['status','adminNotes'] })"
                        ...
                        <button @click="$action.patch('{{ route('admin.donations.inkind.status', $inkind) }}', { include: ['status','adminNotes'] })"
                                class="w-full inline-flex items-center justify-center px-4 py-2.5 rounded-xl text-sm font-semibold"
