...
@section('content')
    ...
    <div x-data="{
            ...
            team_section_visible: {{ Js::from($settings['team_section_visible'] ?? '0') }},
            payments_enabled:    {{ Js::from($settings['payments_enabled'] ?? '0') }}
         }"
        ...
        <div class="bg-white rounded-2xl shadow-sm overflow-hidden" style="border: 1px solid #e2e8f0;">
            ...
            <div class="px-6 py-5 space-y-4">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium" style="color: #374151;">Activer les paiements en ligne (Flutterwave)</p>
                        <p class="text-xs mt-0.5" style="color: #94a3b8;">Affiche les options de paiement par carte sur le site public. Les promesses et dons en nature restent accessibles.</p>
                    </div>
                    <button @click="payments_enabled = payments_enabled === '1' ? '0' : '1'"
                            :style="`background-color: ${payments_enabled === '1' ? '#143c64' : '#e2e8f0'}`"
                            class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200">
                        <span :style="`transform: translateX(${payments_enabled === '1' ? '20px' : '2px'})`"
        ...
        <div class="bg-white rounded-2xl shadow-sm overflow-hidden" style="border: 1px solid #e2e8f0;">
            <div class="px-6 py-4" style="border-bottom: 1px solid #f1f5f9;">
                <h2 class="text-sm font-semibold" style="color: #1e293b;">Sections publiques</h2>
                <p class="text-xs mt-0.5" style="color: #94a3b8;">Contrôler la visibilité des sections sur le site public</p>
            </div>
            <div class="px-6 py-5 space-y-4">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium" style="color: #374151;">Afficher l'équipe (page À propos)</p>
                        <p class="text-xs mt-0.5" style="color: #94a3b8;">Affiche la grille des membres de l'équipe sur la page publique</p>
                    </div>
                    <button @click="team_section_visible = team_section_visible === '1' ? '0' : '1'"
                            :style="`background-color: ${team_section_visible === '1' ? '#143c64' : '#e2e8f0'}`"
                            class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200">
                        <span :style="`transform: translateX(${team_section_visible === '1' ? '20px' : '2px'})`"
                              class="inline-block h-5 w-5 transform rounded-full bg-white shadow-sm transition-transform duration-200"></span>
                    </button>
                </div>
            </div>
        ...
        <div class="flex items-center justify-end gap-3">
            <button @click="$action.patch('{{ route('admin.settings.update') }}', { include: [
                        ...
                        'team_section_visible','payments_enabled'
                    ] })"
