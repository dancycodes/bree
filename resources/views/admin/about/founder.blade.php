...
@section('title', 'Fondatrice — À Propos')
@section('page_title', 'Profil de la fondatrice')
@section('breadcrumb', 'À Propos › Fondatrice')

@section('content')

    <div
        x-data="{
            name: {{ json_encode($founder->name ?? '') }},
            title_fr: {{ json_encode($founder->title_fr ?? '') }},
            title_en: {{ json_encode($founder->title_en ?? '') }},
            bio_fr: {{ json_encode($founder->bio_fr ?? '') }},
            bio_en: {{ json_encode($founder->bio_en ?? '') }},
            message_fr: {{ json_encode($founder->message_fr ?? '') }},
            message_en: {{ json_encode($founder->message_en ?? '') }},
            photo_path: {{ json_encode($founder->photo_path ?? '') }},
            activeLang: 'fr'
        }"
        x-sync="['name','title_fr','title_en','bio_fr','bio_en','message_fr','message_en','photo_path']">
            ...
            <div class="lg:col-span-2 space-y-5">
                ...
                <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <div class="p-6 space-y-5">
                        ...
                        <div x-show="activeLang === 'fr'">
                            <label class="block text-xs font-medium mb-1.5" style="color: #475569;">
                                Biographie (FR) <span style="color: #ef4444;">*</span>
                            </label>
                            <textarea x-model="bio_fr" x-name="bio_fr" rows="8"
                        ...
                        <div x-show="activeLang === 'en'">
                            <label class="block text-xs font-medium mb-1.5" style="color: #475569;">
                                Biography (EN) <span style="color: #ef4444;">*</span>
                            </label>
                            <textarea x-model="bio_en" x-name="bio_en" rows="8"
                                      class="w-full text-sm px-3 py-2.5 rounded-lg border focus:outline-none resize-none"
                                      style="border-color: #e2e8f0; color: #1e293b;"></textarea>
                        ...
                        <div x-show="activeLang === 'fr'">
                            <label class="block text-xs font-medium mb-1.5" style="color: #475569;">
                                Message personnel (FR)
                            </label>
                            <textarea x-model="message_fr" x-name="message_fr" rows="3"
                                      class="w-full text-sm px-3 py-2.5 rounded-lg border focus:outline-none resize-none"
                                      style="border-color: #e2e8f0; color: #1e293b;"></textarea>
                            <p x-message="message_fr" class="text-xs mt-1" style="color: #ef4444;"></p>
                        </div>
                        <div x-show="activeLang === 'en'">
                            <label class="block text-xs font-medium mb-1.5" style="color: #475569;">
                                Personal message (EN)
                            </label>
                            <textarea x-model="message_en" x-name="message_en" rows="3"
                                      class="w-full text-sm px-3 py-2.5 rounded-lg border focus:outline-none resize-none"
                                      style="border-color: #e2e8f0; color: #1e293b;"></textarea>
                            <p x-message="message_en" class="text-xs mt-1" style="color: #ef4444;"></p>
                        </div>
            ...
            <div class="space-y-5">
                ...
                <div class="bg-white rounded-2xl shadow-sm p-5">
                    ...
                    <input x-model="photo_path" x-name="photo_path" type="text"
                           ...
                           style="border-color: #e2e8f0; color: #1e293b;"
                           placeholder="images/sections/fondatrice.jpg">
                    <p x-message="photo_path" class="text-xs mt-1" style="color: #ef4444;"></p>
                </div>

                {{-- Save button --}}
                <button
                    @click="$action.patch('{{ route('admin.about.founder.update') }}')"
                    :disabled="$fetching()"
