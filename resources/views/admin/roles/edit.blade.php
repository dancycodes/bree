...
@section('breadcrumb', 'Rôles › ' . $role->name)

@section('content')

    <div x-data="{
            permissions: {{ Js::from($rolePermissions) }},
            togglePerm(p) {
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="space-y-5">
                ...
                <div class="bg-white rounded-2xl shadow-sm p-6" style="border: 1px solid #e2e8f0;">
                    ...
                    <div class="w-full px-3.5 py-2.5 rounded-xl text-sm font-medium"
                         style="background-color: #f8fafc; border: 1px solid #e2e8f0; color: #475569;">
                        {{ $role->name }}
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-2">
                    <button @click="$action.patch('{{ route('admin.roles.update', $role) }}', { include: ['permissions'] })"
                        ...
                        <span x-show="!$fetching()">Enregistrer les permissions</span>
