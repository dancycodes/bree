...
@section('content')
    <div x-data class="space-y-6">
        ...
        {{-- Metrics (auto-refreshed every 30s via Gale polling) --}}
        @fragment('metrics')
        <div id="metrics"
             x-interval.30s.visible="$action.post('{{ route('admin.dashboard.refresh') }}', { include: [] })">

            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">

                {{-- Card 1: Total Donations (all-time sum) --}}
                @if ($canViewDonations)
                    <a href="{{ route('admin.donations.index') }}"
                       class="bg-white rounded-2xl p-5 flex items-center gap-4 shadow-sm transition-shadow hover:shadow-md"
                       style="border: 1px solid #fce7f3; text-decoration: none;">
                        ...
                        <div class="min-w-0">
                            <p class="text-2xl font-bold truncate" style="color: #1e293b;">
                                {{ number_format($donationsTotalAllTime ?? 0, 0, ',', ' ') }}
                                <span class="text-sm font-medium" style="color: #94a3b8;">XAF</span>
                            </p>
                            <p class="text-xs mt-0.5" style="color: #94a3b8;">{{ __('admin.metric.total_donations') }}</p>
                        </div>
                    </a>
                @else
                    <div class="bg-white rounded-2xl p-5 flex items-center gap-4 shadow-sm" style="border: 1px solid #fce7f3; opacity: 0.4;">
                        <div class="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0"
                             style="background-color: rgba(200,0,120,0.08);">
                            <svg class="w-5 h-5" fill="none" stroke="#c80078" viewBox="0 0 24 24" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                        ...
                        <div class="min-w-0">
                            <p class="text-2xl font-bold truncate" style="color: #1e293b;">0
                                <span class="text-sm font-medium" style="color: #94a3b8;">XAF</span>
                            </p>
                            <p class="text-xs mt-0.5" style="color: #94a3b8;">{{ __('admin.metric.total_donations') }}</p>
                        </div>
                ...
                {{-- Card 2: New Messages (unread) --}}
                <a href="{{ route('admin.messages.index') }}"
                   class="bg-white rounded-2xl p-5 flex items-center gap-4 shadow-sm transition-shadow hover:shadow-md"
                   style="border: 1px solid #eff6ff; text-decoration: none;">
                    <div class="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0"
                         style="background-color: rgba(20,60,100,0.08);">
                        <svg class="w-5 h-5" fill="none" stroke="#143c64" viewBox="0 0 24 24" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/>
                        </svg>
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-2xl font-bold" style="color: #1e293b;">{{ $unreadMessages }}</p>
                        <p class="text-xs mt-0.5" style="color: #94a3b8;">{{ __('admin.metric.new_messages') }}</p>
                    </div>
                    @if ($unreadMessages > 0)
                        <span class="inline-flex items-center justify-center w-5 h-5 rounded-full text-xs font-bold text-white flex-shrink-0"
                              style="background-color: #c80078;">{{ $unreadMessages > 99 ? '99+' : $unreadMessages }}</span>
                    @endif
                </a>
                ...
                <a href="{{ route('admin.events.index') }}"
                   class="bg-white rounded-2xl p-5 flex items-center gap-4 shadow-sm transition-shadow hover:shadow-md"
                   style="border: 1px solid #ecfdf5; text-decoration: none;">
                    <div class="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0"
                    ...
                    <div>
                        ...
                        <p class="text-xs mt-0.5" style="color: #94a3b8;">{{ __('admin.metric.upcoming_events') }}</p>
                    </div>
                </a>

                {{-- Card 4: Recent Activities (count of log entries shown) --}}
                <div class="bg-white rounded-2xl p-5 flex items-center gap-4 shadow-sm"
                     style="border: 1px solid #fef9ec;">
                    <div class="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0"
                    ...
                    <div>
                        <p class="text-2xl font-bold" style="color: #1e293b;">{{ $recentActivity->count() }}</p>
                        <p class="text-xs mt-0.5" style="color: #94a3b8;">{{ __('admin.metric.recent_activities') }}</p>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 bg-white rounded-2xl shadow-sm overflow-hidden" style="border: 1px solid #e2e8f0;">
                <div class="px-5 py-4" style="border-bottom: 1px solid #f1f5f9;">
                    <h3 class="text-sm font-semibold" style="color: #1e293b;">{{ __('admin.recent_activity') }}</h3>
                </div>
                @if ($recentActivity->isEmpty())
                    <div class="py-10 text-center">
                        <p class="text-sm" style="color: #94a3b8;">{{ __('admin.no_activity') }}</p>
                    </div>
                @else
                            ...
                            <li class="px-5 py-3.5 flex items-start gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <p class="text-xs mt-0.5" style="color: #94a3b8;">
                                        {{ $activity->causer?->name ?? __('admin.system') }}
            ...
            <div class="bg-white rounded-2xl shadow-sm overflow-hidden" style="border: 1px solid #e2e8f0;">
                <div class="px-5 py-4" style="border-bottom: 1px solid #f1f5f9;">
                    <h3 class="text-sm font-semibold" style="color: #1e293b;">{{ __('admin.quick_actions') }}</h3>
                </div>
