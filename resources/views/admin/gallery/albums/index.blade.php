...
@section('title', 'Galerie — Albums')
@section('page_title', 'Galerie')
@section('breadcrumb', 'Galerie › Albums')

@section('content')

    <div
        x-data="{ search: {{ Js::from($search) }} }"
        x-navigate.key.albums-table>
        ...
        <div class="flex items-center justify-between mb-6">
            <div class="flex items-center gap-1 p-1 rounded-xl" style="background-color: #f1f5f9;">
                @foreach ([
                    ...
                    <a href="{{ route('admin.gallery.albums.index', array_merge(request()->only('search'), ['status' => $tab])) }}"
                       @click.prevent="$navigate('{{ route('admin.gallery.albums.index', array_merge(request()->only('search'), ['status' => $tab])) }}', { key: 'albums-table', replace: true })"
        ...
        <div class="mb-4">
            <div class="relative max-w-sm">
                ...
                <input
                    ...
                    @input.debounce.400ms="$navigate('{{ route('admin.gallery.albums.index') }}' + '?status={{ $status }}&search=' + encodeURIComponent($event.target.value), { key: 'albums-table', replace: true })"
                    class="w-full pl-9 pr-4 py-2.5 text-sm rounded-xl border focus:outline-none"
                    style="border-color: #e2e8f0; color: #1e293b;">
            </div>
        </div>

        @fragment('albums-table')
        <div id="albums-table">
            ...
            <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
                ...
                @if ($albums->isEmpty())
                    <div class="py-16 text-center">
                        @if ($search !== '')
                            <p class="text-sm" style="color: #94a3b8;">Aucun album ne correspond à « {{ $search }} ».</p>
                            <a href="{{ route('admin.gallery.albums.index', ['status' => $status]) }}"
                               @click.prevent="search = ''; $navigate('{{ route('admin.gallery.albums.index', ['status' => $status]) }}', { key: 'albums-table', replace: true })"
                               class="inline-block mt-2 text-xs font-semibold" style="color: #c80078;">
                                Effacer la recherche
                            </a>
                        @else
                            <p class="text-sm" style="color: #94a3b8;">Aucun album pour le moment.</p>
                            <a href="{{ route('admin.gallery.albums.create') }}" class="inline-block mt-3 text-xs font-semibold"
                               style="color: #c80078;">
                        ...
                        <tbody class="divide-y" style="border-color: #f8fafc;">
                            @foreach ($albums as $album)
                                <tr class="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td class="px-4 py-4 text-xs hidden md:table-cell" style="color: #475569;">
                                        {{ $album->photos_count }}
                                    </td>
                                    <td class="px-4 py-4 text-xs hidden lg:table-cell" style="color: #64748b;">
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex items-center gap-1 justify-end">
                                            ...
                                            @endif
                                            <button
                                                @click="if (confirm('Supprimer cet album et toutes ses photos ?')) $action.delete('{{ route('admin.gallery.albums.destroy', $album) }}')"
                        ...
                        <div class="px-6 py-4 border-t flex items-center justify-center gap-2"
                            ...
                            @if (! $albums->onFirstPage())
                                <a href="{{ $albums->previousPageUrl() }}"
                                   @click.prevent="$navigate('{{ $albums->previousPageUrl() }}', { key: 'albums-table', replace: true })"
                            ...
                            @foreach ($albums->getUrlRange(max(1, $albums->currentPage() - 2), min($albums->lastPage(), $albums->currentPage() + 2)) as $page => $url)
                                ...
                                @else
                                    <a href="{{ $url }}"
                                       @click.prevent="$navigate('{{ $url }}', { key: 'albums-table', replace: true })"
                                       class="px-3 py-1.5 rounded-lg text-xs transition-colors"
                            ...
                            @if ($albums->hasMorePages())
                                <a href="{{ $albums->nextPageUrl() }}"
                                   @click.prevent="$navigate('{{ $albums->nextPageUrl() }}', { key: 'albums-table', replace: true })"
                                   class="px-3 py-1.5 rounded-lg text-xs transition-colors"
